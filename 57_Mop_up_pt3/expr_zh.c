...
static struct ASTnode *paren_expression(int ptp) {
  ...
  // 如果之后的标记是类型标识符，这是类型转换表达式
  switch (Token.token) {
  case T_IDENT:
    // 我们必须检查标识符是否匹配typedef。
    // 如果不是，则视为表达式。
    if (findtypedef(Text) == NULL) {
      n = binexpr(0);	// 因为括号内的表达式ptp为零
      break;
    }
  case T_VOID:
  case T_CHAR:
  ...
  default:
    n = binexpr(ptp);		// 扫描表达式。我们传入ptp
				// 因为类型转换不会改变
				// 表达式的优先级
  }

  // 我们现在在n中至少有一个表达式，如果存在类型转换，
  // 则可能在type中有非零类型。
  // 如果没有类型转换则跳过 ')'。
  if (type == 0)
    rparen();
...
static struct ASTnode *primary(int ptp) {
  ...
  case T_INTLIT:
    // 对于 INTLIT 标记，制作一个叶AST节点。
    // 如果在 P_CHAR 范围内则使其为 P_CHAR
...
struct ASTnode *binexpr(int ptp) {
  ...
  while ((op_precedence(tokentype) > ptp) ||
    ...
    default:
      // 我们不是做三元或赋值，所以两个树都应该是
      //rvalue。如果它们是lvalue树则转换为rvalue
      left->rvalue = 1;
      right->rvalue = 1;
