...
#define TABLE_INCREMENT 10
    ...
    // 如果数组已有nelems，分配该数量的元素
    // 在列表中。否则，从TABLE_INCREMENT开始。
    if (nelems != -1)
      maxelems = nelems;
    else
...
// 要么找到现有的struct/union声明，要么构建
// 一个struct/union符号表条目并返回其指针。
static struct symtable *composite_declaration(int type) {
  ...
  // 设置组合类型中每个后续成员的位置
  // union很简单。对于struct，对齐成员并找到下一个空闲字节
