...
// 解析当前标记并返回基本类型枚举值,
// 指向任何组合类型的指针,以及可能修改类型的类
int parse_type(struct symtable **ctype, int *class) {
  int type, exstatic = 1;

  // 查看类是否已更改为extern或static
  while (exstatic) {
    switch (Token.token) {
      case T_EXTERN:
	if (*class == C_STATIC)
...
static struct symtable *function_declaration(char *funcname, int type,
  ...
  // Text有标识符的名称。如果存在且是
  // 函数,则获取id。否则,将oldfuncsym设为NULL
  if ((oldfuncsym = findsymbol(funcname)) != NULL)
    if (oldfuncsym->stype != S_FUNCTION)
      oldfuncsym = NULL;

  // 如果这是一个新的函数声明,获取一个
  // 用于结束标签的标签id,并将函数添加到符号表
  if (oldfuncsym == NULL) {
    endlabel = genlabel();
    // 假设:函数只返回标量类型,所以下面用NULL
    newfuncsym =
