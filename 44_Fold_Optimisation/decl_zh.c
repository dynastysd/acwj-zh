...
// 给定一个类型，检查最新标记是否是
// 该类型的字面量。如果是整数字面量，返回此值。
// 如果是字符串字面量，返回字符串的标签号。
// 不扫描下一个标记。如果类型是 P_NONE，则放松
// 所有解析限制
...
static struct symtable *function_declaration(char *funcname, int type,
  ...
  // Text 中有标识符的名称。如果存在且是
  // 函数，获取 id。否则，将 oldfuncsym 设置为 NULL。
