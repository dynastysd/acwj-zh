...
static int next(void) {
  ...
  while (c == '#') {		// 遇到预处理器语句
    scan(&Token);		// 将行号读取到 l 中
    if (Token.token != T_INTLIT)
      fatals("Expecting pre-processor line number, got:", Text);
    l = Token.intvalue;

    scan(&Token);		// 将文件名读取到 Text 中
    if (Token.token != T_STRLIT)
      fatals("Expecting pre-processor file name, got:", Text);

    if (Text[0] != '<') {	// 如果是真实的文件名
      if (strcmp(Text, Infilename))	// 而且不是当前的文件
	Infilename = strdup(Text);	// 就保存它。然后更新行号
      Line = l;
    }
...
// 根据输入的单词，返回匹配的
// 关键字标记号，如果不是关键字则返回 0。
// 对第一个字母进行 switch 以节省时间，
// 避免与所有关键字进行 strcmp() 比较。
