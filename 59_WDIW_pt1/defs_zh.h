...
#define st_endlabel st_posn	// 对于函数,结束标签
  ...
  int *initlist;		// 初始值列表
  struct symtable *next;	// 指向下一个符号的指针
  struct symtable *member;	// 函数、struct 的第一个成员,
#ifdef __NASM__
  int extinit;
#endif
};				// union 或 enum
...
struct ASTnode {
  ...
  struct symtable *sym;	// 对于许多 AST 节点,指向符号表中符号的指针
#define a_intvalue a_size	// 对于 A_INTLIT,整数值
  int a_size;			// 对于 A_SCALE,要缩放的大小
  int linenum;			// 此节点所在行的行号
};

enum {
  NOREG = -1,			// 当 AST 生成函数没有寄存器可返回时使用 NOREG
  NOLABEL = 0			// 当我们没有标签传递给 genAST() 时使用 NOLABEL
};