...
// 解析零个或多个逗号分隔的表达式列表,
// 返回由 A_GLUE 节点组成的 AST,左子节点是
// 先前表达式的子树(或 NULL),右子节点是下一个表达式。
...
// 每个标记的运算符优先级。必须与 defs.h 中的标记顺序匹配
static int OpPrec[] = {
...
static struct ASTnode *prefix(int ptp) {
  ...
  case T_MINUS:
    ...
    // 在树的开头添加一个 A_NEGATE 操作,
    // 并将子节点设为右值。因为 char 是无符号的,
    // 如果需要也将其扩展为 int 以便有符号
    tree->rvalue = 1;
    if (tree->type == P_CHAR)
      tree->type = P_INT;
