...
// 对具有二元运算符和两个 A_INTLIT 子树的 AST 树进行折叠。
// 返回原始树或新的叶子节点。
...
// 对具有一元运算符和一个 INTLIT 子树的 AST 树进行折叠。
// 返回原始树或新的叶子节点。
