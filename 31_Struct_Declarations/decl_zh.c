...
// 当解析结构体/联合体的成员时，separate_token是';'。
//
// 解析变量列表
...
struct ASTnode *function_declaration(int type) {
  ...
  // Text有标识符的名称。如果存在且是
  // 函数，获取其id。否则，将oldfuncsym设置为NULL
  ...
  if (type != P_VOID) {
    ...
    // 如果函数中没有语句则报错
    if (tree == NULL)
      fatal("非void类型的函数中没有语句");

    // 检查复合语句中
    // 最后一个AST操作是否是return语句
    finalstmt = (tree->op == A_GLUE) ? tree->right : tree;
    if (finalstmt == NULL || finalstmt->op != A_RETURN)
      fatal("非void类型的函数没有return");
  }
  // 返回A_FUNCTION节点，它有函数的符号指针
  // 和复合语句子树
...
static struct symtable *struct_declaration(void) {
  ...
  if (Token.token != T_LBRACE) {
    if (ctype == NULL)
      fatals("未知的结构体类型", Text);
    return (ctype);
  }
  // 确保此结构体类型尚未被
  // 之前定义过
  if (ctype)
    fatals("之前已定义的结构体", Text);
