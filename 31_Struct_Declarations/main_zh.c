...
// 给定一个包含'.'和至少一个1字符后缀的字符串，
// 将后缀更改为给定字符。
// 返回新字符串，如果原始字符串无法修改则返回NULL
...
static char *do_compile(char *filename) {
  ...
  if (Outfilename == NULL) {
    fprintf(stderr, "错误: %s 没有后缀，请使用 .c 结尾\n", filename);
    exit(1);
  }
  // 打开输入文件
...
char *do_assemble(char *filename) {
  ...
  if (outfilename == NULL) {
    fprintf(stderr, "错误: %s 没有后缀，请使用 .s 结尾\n", filename);
    exit(1);
  }
  // 构建汇编命令并运行
...
static void usage(char *prog) {
  ...
  fprintf(stderr, "       -o outfile, 生成outfile可执行文件\n");
  exit(1);
}

// 主程序: 检查参数，如果没有参数则打印用法
// 打开输入文件并调用scanfile()扫描其中的标记
enum { MAXOBJ = 100 };
int main(int argc, char *argv[]) {
  ...
  while (i < argc) {
	...
	exit(1);
      ...
      objlist[objcnt++] = objfile;	// 将目标文件名添加到列表
      objlist[objcnt] = NULL;
    }

    if (!O_keepasm)		// 如果不需要保留汇编文件则删除
      unlink(asmfile);
    i++;
  }
