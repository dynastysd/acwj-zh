...
// 之前表达式的子树（或 NULL），右子节点是下一个表达式。
// 每个 A_GLUE 节点的 size 字段会被设置为
// 此时树中表达式的数量。如果没有解析
// 表达式，则返回 NULL
...
static struct ASTnode *primary(void) {
  ...
  case T_LPAREN:
    ...
    // 现在我们至少有 n 中的一个表达式，如果之前有强制转换，
    // type 中可能有非零类型。如果没有强制转换，
    // 跳过闭 ')'；否则，为强制转换创建一个一元 AST 节点。
    if (type == 0)
      rparen();
    else
