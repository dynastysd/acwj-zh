...
// 解析零个或多个逗号分隔的表达式列表，
// 返回由 A_GLUE 节点组成的 AST，左子节点是之前表达式的子树（或 NULL），
// 右子节点是下一个表达式。每个 A_GLUE 节点的大小字段
// 设置为此时树中的表达式数量。如果没有解析表达式，则返回 NULL
...
static struct ASTnode *primary(void) {
  ...
  case T_LPAREN:
    ...
    // type 可能非零。如果没有强制转换，则跳过 ')'。
    if (type == 0)
      rparen();
    else
      // 否则，为强制类型转换创建一个一元 AST 节点
