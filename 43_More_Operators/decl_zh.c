...
// 给定一个类型，检查最新标记是否是该类型的字面量。
// 如果是整数字面量，返回此值。
// 如果是字符串字面量，返回字符串的标签号。
// 不扫描下一个标记。如果类型是 P_NONE，则放宽所有
// 解析限制
