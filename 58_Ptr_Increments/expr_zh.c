...
// 解析零个或多个逗号分隔的表达式列表，并返回
// 由 A_GLUE 节点组成的 AST，左子节点是先前表达式的
// 子树（或 NULL），右子节点是下一个表达式。
// 每个 A_GLUE 节点的 size 字段将设置为此时
// 树中表达式的数量。如果未解析表达式，则返回 NULL
...
static struct ASTnode *member_access(struct ASTnode *left, int withpointer) {
  ...
  // 如果是，将其从 A_IDENT 更改为 A_ADDR，
  // 这样我们获取的是基址，而不是该地址处的值。
  if (!withpointer) {
...
static struct ASTnode *paren_expression(int ptp) {
  ...
  // 我们现在至少在 n 中有一个表达式，如果有强制转换，
  // 在 type 中可能有非零类型。跳过 ')' 如果没有强制转换。
  if (type == 0)
    rparen();
  else
...
struct ASTnode *binexpr(int ptp) {
  ...
  while ((op_precedence(tokentype) > ptp) ||
    ...
    default:
      // 我们不是做三元或赋值，所以两棵树都应该是
      // rvalue。如果它们是 lvalue 树，则将它们转换为 rvalue
      left->rvalue = 1;
      right->rvalue = 1;
