...
// 解析零个或多个逗号分隔的表达式并返回
// 由 A_GLUE 节点组成的 AST，左子节点是先前表达式的子树（或 NULL），
// 右子节点是下一个表达式。每个 A_GLUE 节点将 size 字段设置为
// 此时树中表达式的数量。如果未解析表达式，则返回 NULL
...
static struct ASTnode *paren_expression(int ptp) {
  ...
  // 我们现在在 n 中至少有一个表达式，如果有强制转换且
  // type 非零。如果没有强制转换，则跳过 ')'。
  if (type == 0)
    rparen();
  else
...
static struct ASTnode *prefix(int ptp) {
  ...
  switch (Token.token) {
    ...
    case T_MINUS:
      ...
      // 在树前添加一个 A_NEGATE 操作并
      // 使子节点成为 rvalue。因为字符是无符号的，
      // 还需要将其扩展为 int 以使其有符号
      tree->rvalue = 1;
      if (tree->type == P_CHAR)
	tree->type = P_INT;
...
struct ASTnode *binexpr(int ptp) {
  ...
  while ((op_precedence(tokentype) > ptp) ||
	...
	// 是 rvalue。如果它们是 lvalue 树则将两者都转换为 rvalue
	left->rvalue = 1;
	right->rvalue = 1;
