...
// 给定标量类型、现有内存偏移量
// （尚未分配给任何内容）和方向（1 向上，-1 向下），
// 计算并返回适合此标量类型的内存偏移量。
// 这可能是原始偏移量，也可能是原始偏移量的上方或下方
int cgalign(int type, int offset, int direction) {
  int alignment;

  // 在 x86-64 上我们不需要这样做，但让
  // 字符在任何偏移量上对齐，int/指针
  // 在 4 字节对齐上对齐
  switch (type) {
    case P_CHAR:
...
#define FIRSTPARAMREG 9		// 第一个形参寄存器在上述寄存器列表中的位置
