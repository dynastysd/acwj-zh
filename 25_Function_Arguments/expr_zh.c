...
// 解析零个或多个逗号分隔的表达式列表并
// 返回由 A_GLUE 节点组成的 AST，左子节点是先前
// 表达式的子树（或者 NULL），右子节点是下一个表达式。
// 每个 A_GLUE 节点的 size 字段被设置为此刻
