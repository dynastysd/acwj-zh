...
// 给定parse_type()解析的类型，扫描任何后续的'*'标记并返回新类型
...
// 给定一个可能已存在的符号指针，
// 如果此符号不存在则返回true。我们使用
// 此函数将extern转换为globals
int is_new_symbol(struct symtable *sym, int class,
  ...
  if ((sym->class== C_GLOBAL && class== C_EXTERN)
      ...
      // Struct/unions，也比较ctype
      if (type >= P_STRUCT && ctype != sym->ctype)
        fatals("Type mismatch between global/extern", sym->name);

      // 如果到这里，类型匹配，所以将符号标记为global
      sym->class= C_GLOBAL;
      // 返回符号不是新的
      return(0);
  }
...
#define TABLE_INCREMENT 10
    ...
    // 如果数组已有nelems，在列表中分配那么多元素。
    // 否则，从TABLE_INCREMENT开始。
    if (nelems != -1)
      maxelems = nelems;
    else
...
static struct symtable *function_declaration(char *funcname, int type,
  ...
  if (type != P_VOID) {
    ...
    // 检查复合语句中的
    // 最后一个AST操作是否是return语句
    finalstmt = (tree->op == A_GLUE) ? tree->right : tree;
    if (finalstmt == NULL || finalstmt->op != A_RETURN)
      fatal("No return for function with non-void type");
  }
  // 构建A_FUNCTION节点，该节点具有函数的符号指针
  // 和复合语句子树
...
// 解析组合类型声明：struct或union。
// 要么找到现有的struct/union声明，
// 要么构建struct/union符号表条目并返回其指针。
static struct symtable *composite_declaration(int type) {
  ...
  // 如果下一个标记不是LBRACE，这是
  // 现有struct/union类型的使用。返回类型的指针。
  if (Token.token != T_LBRACE) {
    if (ctype == NULL)
      fatals("unknown struct/union type", Text);
    return (ctype);
  }
  // 确保此struct/union类型之前没有定义
  if (ctype)
  ...
  // 在组合类型中设置每个后续成员的位置。
  // Union很容易。对于struct，对齐成员并找到下一个可用字节
