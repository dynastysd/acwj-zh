...
// 每个A_GLUE节点的size字段设置为此时树中表达式的数量。
// 如果没有解析表达式，则返回NULL
...
static struct ASTnode *primary(void) {
  ...
  case T_INTLIT:
    // 对于INTLIT标记，为其制作一个叶AST节点。
    // 如果在P_CHAR范围内，则将其设为P_CHAR
  ...
  case T_IDENT:
    ...
    // 查看此标识符是否作为符号存在。对于数组，将rvalue设为1。
...
struct ASTnode *binexpr(int ptp) {
  ...
  while ((op_precedence(tokentype) > ptp) ||
    ...
    default:
      ...
      // 是rvalue。如果它们是lvalue树，则将它们转换为rvalue
      left->rvalue = 1;
      right->rvalue = 1;

      // 通过尝试将每个树修改为匹配对方的类型来确保两种类型兼容。
