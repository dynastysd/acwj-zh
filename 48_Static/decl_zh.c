...
// 给定 parse_type() 解析的类型，扫描后续的 '*' 标记并返回新类型
...
#define TABLE_INCREMENT 10
    ...
    // 如果数组已经有 nelems 个元素，
    // 在列表中分配那么多元素。否则，从 TABLE_INCREMENT 开始。
    if (nelems != -1)
      maxelems= nelems;
    else
...
// 给定新函数的指针和可能指向该函数前一个声明的指针，
// 解析参数列表并根据前一个声明进行交叉检查。
// 返回参数的数量
...
static struct symtable *function_declaration(char *funcname, int type,
  ...
  // Text 有标识符的名称。如果存在且是
  // 函数，获取其 id。否则，将 oldfuncsym 设为 NULL。
