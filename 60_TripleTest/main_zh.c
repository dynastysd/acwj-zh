...
// 给定一个带有 '.' 和至少一个 1 字符后缀的字符串，
// 将后缀更改为给定字符。
// 返回新字符串，如果原始字符串不能被修改则返回 NULL
...
static char *do_compile(char *filename) {
  ...
  Peektoken.token = 0;		// 且没有前瞻标记
  genpreamble(filename);	// 输出前导码
  ...
  freestaticsyms();		// 释放文件中任何 static 符号
  return (Outfilename);
}
...
int main(int argc, char **argv) {
  ...
  while (i < argc) {
    ...
    if (O_dolink || O_assemble) {
      objfile = do_assemble(asmfile);	// 将其汇编为对象格式
      if (objcnt == (MAXOBJ - 2)) {
