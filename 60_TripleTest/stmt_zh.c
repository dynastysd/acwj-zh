...
static struct ASTnode *switch_statement(void) {
  ...
  while (inloop) {
	...
	break;
      case T_CASE:
      case T_DEFAULT:
	// 确保这不是在 previous 'default' 之后
	if (seendefault)
	  fatal("case or default after existing default");

	// 设置 AST 操作。如需要则扫描 case 值
	...
	// 如果下一个标记是 T_CASE，现有 case 将 fall
	// 进入下一个 case。否则，解析 case body。
  ...
  // 我们有一个带有 cases 和任何 default 的子树。将
  // case 计数放入 A_SWITCH 节点并附加 case 树。
