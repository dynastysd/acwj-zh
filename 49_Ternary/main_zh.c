...
// 给定一个包含'.'和至少1个字符后缀的字符串，
// 将后缀更改为给定字符。
// 如果原始字符串无法修改则返回新字符串或NULL
...
static char *do_compile(char *filename) {
  ...
  if (O_verbose)
    printf("compiling %s\n", filename);
  scan(&Token);			// 从输入获取第一个词素
  Peektoken.token= 0;		// 并设置没有前瞻词素
  genpreamble();		// 输出前导码
  global_declarations();	// 解析全局声明
  genpostamble();		// 输出后置码
  fclose(Outfile);		// 关闭输出文件
  freestaticsyms();		// 释放文件中任何静态符号
  return (Outfilename);
}
...
// 主程序：检查参数，如果我们没有参数则打印用法。
// 打开输入文件并调用scanfile()来扫描其中的词素。
enum { MAXOBJ = 100 };
int main(int argc, char *argv[]) {
  ...
  while (i < argc) {
	...
	exit(1);
      ...
      objlist[objcnt++] = objfile;	// 将目标文件的名称
      objlist[objcnt] = NULL;	// 添加到目标文件列表
    }

    if (!O_keepasm)		// 如果不需要保留汇编文件则删除它
      unlink(asmfile);
    i++;
  }
