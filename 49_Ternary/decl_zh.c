...
#define TABLE_INCREMENT 10
    ...
    // 如果数组已经有nelems个元素，则分配那么多
    // 元素在列表中。否则，从TABLE_INCREMENT开始。
    if (nelems != -1)
      maxelems= nelems;
    else
...
static struct symtable *function_declaration(char *funcname, int type,
  ...
  // Text有标识符的名称。如果存在且是
  // 函数，则获取id。否则，将oldfuncsym设置为NULL。
