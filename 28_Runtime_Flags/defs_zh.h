...
struct ASTnode {
  ...
  struct ASTnode *left;		// 左、中、右子树的指针
  struct ASTnode *mid;
...
#define NOLABEL	 0		// 当我们没有标签要传递给 genAST() 时使用 NOLABEL
...
struct symtable {
  ...
  int stype;			// 符号的结构类型
  int class;			// 符号的存储类别
  int endlabel;			// 对于 S_FUNCTION，存储结束标签
  int size;			// 符号中元素的数量
  int posn;			// 对于局部变量，或者是栈基址指针的负偏移量，或者是寄存器 ID
#define nelems posn		// 对于函数，存储参数数量
				// 对于结构体，存储字段数量
};
