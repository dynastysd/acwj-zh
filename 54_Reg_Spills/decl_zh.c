...
// 给定由 parse_type() 解析的类型，扫描后续的 '*' 标记并返回新类型
...
static struct symtable *function_declaration(char *funcname, int type,
  ...
  // 则获取 id。否则，将 oldfuncsym 设为 NULL。
  if ((oldfuncsym = findsymbol(funcname)) != NULL)
    if (oldfuncsym->stype != S_FUNCTION)
      oldfuncsym = NULL;

  // 如果这是新函数声明，获取
  // 结束标签的标签 id，并将函数添加到符号表，
  if (oldfuncsym == NULL) {
    endlabel = genlabel();
    // 假设函数只返回标量类型，所以下面为 NULL
    newfuncsym =
