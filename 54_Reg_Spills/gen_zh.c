...
static int gen_logandor(struct ASTnode *n) {
  ...
  // 生成左表达式的代码，后跟跳转到假标签的跳转
  reg= genAST(n->left, NOLABEL, NOLABEL, NOLABEL, 0);
  cgboolean(reg, n->op, Lfalse);
  genfreeregs(NOREG);

  // 生成右表达式的代码，后跟跳转到假标签的跳转
