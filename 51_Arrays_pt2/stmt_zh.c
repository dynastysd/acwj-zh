...
//
// 解析 IF 语句包括任何可选 ELSE 子句并返回其 AST
...
static struct ASTnode *switch_statement(void) {
  ...
  while (inloop) {
	...
	// 如果下一个标记是 T_CASE，现有的 case 将落入
	// 下一个 case。否则，解析 case body。
