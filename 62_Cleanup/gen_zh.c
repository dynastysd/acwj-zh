...
int genAST(struct ASTnode *n, int iflabel, int looptoplabel,
  ...
  case A_GE:
    ...
    // 生成比较后跟跳转。否则，比较寄存器并根据比较结果设置为 1 或 0
  ...
  case A_ASSIGN:
    ...
    // 对于 '+=' 及类似运算符，生成合适的代码并获取结果的寄存器。
    // 然后获取左子节点，使其成为右子节点，以便我们可以进入赋值代码
    switch (n->op) {
    case A_ASPLUS:
  ...
  case A_DEREF:
    ...
    // 否则将其保留以供 A_ASSIGN 通过指针存储
    if (n->rvalue)
      return (cgderef(leftreg, n->left->type));
    else
      return (leftreg);
  case A_SCALE:
    // 小优化：如果缩放值是 2 的已知幂，则使用移位
    switch (n->a_size) {
    case 2:
  ...
  case A_TOBOOL:
    // 如果父 AST 节点是 A_IF 或 A_WHILE，生成比较后跟跳转。
    // 否则，根据其为零性或非零性将寄存器设置为 0 或 1
...
// 生成全局字符串。如果 append 为 true，
// 追加到之前的 genglobstr() 调用
