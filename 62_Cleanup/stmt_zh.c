...
static struct ASTnode *switch_statement(void) {
  ...
  if (!inttype(left->type))
    fatal("Switch 表达式不是整数类型");

  // 使用表达式作为子节点构建 A_SWITCH 子树
  ...
  while (inloop) {
	...
	if (seendefault)
	  fatal("在现有 default 之后的 case 或 default");

	// 设置 AST 操作。如需要，扫描 case 值
	...
	// 如果下一个标记是 T_CASE，则现有 case 将落入下一个 case。
	// 否则，解析 case 体。
	...
	break;
      default:
	fatals("switch 中意外的标记", Token.tokstr);
    }
  }
  Switchlevel--;

  // 我们有一个包含 case 和任何 default 的子树。
  // 将 case 数量放入 A_SWITCH 节点并附加 case 树。
