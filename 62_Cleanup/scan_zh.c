...
static int scanch(void) {
  ...
  if (c == '\\') {
	...
	// 处理八进制常量：读取字符直到遇到非八进制数字。
	// 在 c2 中累加八进制值，统计数字个数 i。
	// 只允许 3 个八进制数字。
      case '0':
      case '1':
      case '2':
...
// 给定输入中的一个词，返回匹配的关键字标记号，
// 如果不是关键字则返回 0。
// 根据第一个字母进行切换，这样我们就不必
// 通过 strcmp() 与所有关键字比较浪费时间
