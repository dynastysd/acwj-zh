...
int cgstorlocal(int r, struct symtable *sym) {
  ...
  switch (sym->type) {
    ...
    case P_INT:
      ...
      break;
    default:
      fatald("cgstorlocal 中的错误类型：", sym->type);
  }
  return (r);
}

// 给定一个标量类型、一个现有的内存偏移量
//（尚未分配给任何内容）和一个方向（1 是向上，-1 是向下），
// 计算并返回一个适合此标量类型的内存偏移量。
...
// 当我们需要溢出寄存器时，我们选择
// 以下寄存器，然后循环遍历其余寄存器。
// spillreg 不断增加，所以我们需要对其取模 NUMFREEREGS。
...
// 将布尔值（仅 0 或 1）加载到给定寄存器中
void cgloadboolean(int r, int val) {
  fprintf(Outfile, "\tmov\t%s, %d\n", reglist[r], val);
}

// 将整数值转换为布尔值。如果是 IF、WHILE、LOGAND 或 LOGOR 操作，则跳转
...
// 给定带有参数值的寄存器，
// 将此参数复制到 argposn'th 参数中，
// 以为将来的函数调用做准备。
// 注意 argposn 是 1、2、3、4、...，永远不会为零。
void cgcopyarg(int r, int argposn) {

  // 如果超过第六个参数，只需将寄存器压入栈。
  // 我们依赖以正确的顺序调用带有连续参数的函数
  if (argposn > 6) {
    fprintf(Outfile, "\tpush\t%s\n", reglist[r]);
  } else {
...
int cgstorglob(int r, struct symtable *sym) {
  ...
  switch (sym->type) {
    ...
    case P_INT:
      ...
      break;
    default:
      fatald("cgloadglob 中的错误类型：", sym->type);
  }
  return (r);
}
...
int cgcompare_and_set(int ASTop, int r1, int r2, int type) {
  ...
  if (ASTop < A_EQ || ASTop > A_GE)
    fatal("cgcompare_and_set() 中的错误 ASTop");
    ...
    if (ptrtype(sym->type))
      ...
      switch (sym->type) {
        ...
        case P_LONG:
          ...
          break;
        default:
          fatald("cgreturn 中的错误函数类型：", sym->type);
      }
    }
  }
...
int cgstorderef(int r1, int r2, int type) {
  ...
  switch (size) {
    ...
    case 8:
      ...
      break;
    default:
      fatald("无法在 cgstoderef 上进行操作：", type);
  }
  return (r1);
}
