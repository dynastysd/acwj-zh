...
// 给定新函数的指针和可能为NULL的函数先前声明的指针，
// 解析参数列表并根据先前的声明进行交叉检查。返回参数数量
...
static struct symtable *function_declaration(char *funcname, int type,
  ...
  // Text有标识符的名称。如果存在且是函数，获取id。
  // 否则，将oldfuncsym设为NULL。
...
static struct symtable *composite_declaration(int type) {
  ...
  // 如果下一个标记不是LBRACE，这只是
  // 现有struct/union类型的使用。返回类型指针。
  if (Token.token != T_LBRACE) {
    if (ctype == NULL)
      fatals("unknown struct/union type", Text);
    return (ctype);
  }
  // 确保此struct/union类型尚未被定义
  if (ctype)
  ...
  // 设置组合类型中每个连续成员的位置。
  // 联合体很容易。对于struct，对齐成员并找到下一个空闲字节
...
// 解析有初始类型的符号列表。返回符号的类型。et1和et2是结束标记。
