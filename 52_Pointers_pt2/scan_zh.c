...
static int next(void) {
  ...
  while (Linestart && c == '#') {	// 我们遇到了预处理语句
    ...
    scan(&Token);		// 获取文件名到Text
    if (Token.token != T_STRLIT)
      fatals("Expecting pre-processor file name, got:", Text);

    if (Text[0] != '<') {	// 如果是真实的文件名
      if (strcmp(Text, Infilename))	// 并且不是当前文件名
