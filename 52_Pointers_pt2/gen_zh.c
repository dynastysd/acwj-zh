...
static int genSWITCH(struct ASTnode *n) {
  ...
  for (i = 0, c = n->right; c != NULL; i++, c = c->right) {
    ...
    // 生成case代码。传入结束的break标签。
    // 如果case没有body，我们将进入下面的body。
    if (c->left)
...
int genAST(struct ASTnode *n, int iflabel, int looptoplabel,
  ...
  switch (n->op) {
    ...
    case A_GE:
      ...
      // 生成比较后跟跳转。否则，比较寄存器并根据比较结果设置为1或0。
    ...
    case A_TOBOOL:
      // 如果父AST节点是A_IF或A_WHILE，
      // 生成比较后跟跳转。否则，根据其为零或非零设置为0或1。
