...
static struct ASTnode *member_access(struct ASTnode *left, int withpointer) {
  ...
  // 检查左 AST 树是否为 struct 或 union 的指针
...
static struct ASTnode *primary(void) {
  ...
  case T_IDENT:
    ...
    // 查看此标识符是否存在作为符号。对于数组，设置 rvalue 为 1。
...
struct ASTnode *prefix(void) {
  ...
  case T_MINUS:
    ...
    // 将 A_NEGATE 操作前置到树，
    // 并使子节点成为 rvalue。因为 char 是无符号的，
    // 如果需要也将其拓宽为 int 以使其有符号
    tree->rvalue = 1;
    if (tree->type == P_CHAR)
      tree->type = P_INT;
...
struct ASTnode *binexpr(int ptp) {
  ...
  while ((op_precedence(tokentype) > ptp) ||
    ...
    default:
      // 我们不是在做三元或赋值，所以两个树都应该是 rvalue。
      // 如果它们是 lvalue 树，则将两者都转换为 rvalue
      left->rvalue = 1;
      right->rvalue = 1;

      // 通过尝试修改每个树以匹配另一者的类型来确保两种类型兼容。
